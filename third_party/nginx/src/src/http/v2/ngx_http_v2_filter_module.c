...
ngx_http_v2_create_trailers_frame(ngx_http_request_t *r)
{
    ...
    for (i = 0; /* void */; i++) {
        ...
        if (header[i].value.len > NGX_HTTP_V2_MAX_FIELD) {
            ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0,
                          "ignoring too long response trailer value: \"%V: %V\"",
                          &header[i].key, &header[i].value);
            continue;
        }
        ...
        /* TODO(piotrsikora): check if encoding can take more than len */
        if (header[i].key.len > tmp_len) {
            tmp_len = header[i].key.len;
        }
...
ngx_http_v2_send_chain(ngx_connection_t *fc, ngx_chain_t *in, off_t limit)
{
    ...
    for ( ;; ) {
        ...
        if (cl->buf->last_buf && 1 /* TODO(piotrsikora): trailers_ok */) {

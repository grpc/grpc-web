...
nginx_config:
	...
	--with-ld-opt="-L /usr/local/lib -lgrpc++ -lgrpc -lprotobuf \
...
nginx_config_static:
	...
	--with-http_v2_module \
	--with-cc-opt="-I /usr/local/include -I $(ROOT_DIR)" \
	--with-ld-opt="-L /usr/local/lib -l:libgrpc++.a -l:libgrpc.a \
...
package_static: nginx_static
	...
	cp $(ROOT_DIR)/third_party/nginx/src/conf/* $(ROOT_DIR)/gConnector_static/conf
	cp $(ROOT_DIR)/net/grpc/gateway/nginx/package/nginx.conf $(ROOT_DIR)/gConnector_static/conf
	cp $(ROOT_DIR)/net/grpc/gateway/nginx/package/nginx.sh $(ROOT_DIR)/gConnector_static
	cp $(ROOT_DIR)/third_party/nginx/src/objs/nginx $(ROOT_DIR)/gConnector_static
...
clean:
	rm -rf objs gConnector gConnector_static third_party/nginx/src/objs third_party/openssl/.openssl
	rm -f gConnector.zip gConnector_static.zip $(PROTOS_DIR)/*.pb.cc $(PROTOS_DIR)/*.pb.h third_party/nginx/src/Makefile
